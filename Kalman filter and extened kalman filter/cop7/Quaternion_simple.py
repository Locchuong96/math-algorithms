...
class System:
	def __init__(self):
		...
		self.yHatBar = np.zeros(3).transpose() # the priori estimate of measurement
		self.P = np.identity(7) * 0.01  # the posteri estimate od the error at time step k
		self.Q = np.identity(7) * 0.001 # the process variance matrix
		self.R = np.identity(6) * 0.1 # the measurement variance
		self.K = None  # the kalman filter
		self.A = None # the tranmission matrix 
		self.B = None # control matrix
		self.C = None  # the measurement matrix
		self.xHatBar = None # the priori estimate of x at time step k
		self.xHatPrev = None 
		self.PBar = None 
		...
		# self.mag_Ainv = np.array([[0.45938117, -0.00493019, -0.00182613], # Magneto matrix Ainv
		# 							[-0.00493019, 0.44225463, 0.02901676],
		# 							[-0.00182613, 0.02901676, 0.47718223]])

		# self.mag_b = np.array([[2.03141429],
		# 					[4.67190737],
		# 					[-3.72005703]])
	...
	def predictAccelMag(self):
		...
		accelBar = getRotMat(self.xHatBar).transpose() @ self.accelReference 

		#Mag
		hPrime_m = self.getJacobianMatrix(self.magReference)
		magBar = getRotMat(self.xHatBar).transpose() @ self.magReference
	...
	def predict(self,w,dt):
		...
		self.xHatBar = ( self.A @ self.xHat) + (self.B @ np.array(w).transpose())
		self.xHatBar[0:4] = self.normalizeQuat(self.xHatBar[0:4])
		self.xHatPrev = self.xHat

		self.yHatBar = self.predictAccelMag()
		self.PBar = ( self.A @ self.P ) @ self.A.transpose() + self.Q

	def update(self,a,m):
		tmp1 = np.linalg.inv( (self.C @ self.PBar) @ self.C.transpose() + self.R )
		self.K = ( self.PBar @ self.C.transpose() ) @ tmp1 

		magGauss_B = self.getMagVector(m)
		accel_B = self.getAccelVector(a)

		measurement = np.concatenate((accel_B,magGauss_B),axis = 0)
		self.xHat = self.xHatBar + self.K @ (measurement - self.yHatBar)
		self.xHat[0:4] = self.normalizeQuat(self.xHat[0:4])
		self.P = (np.identity(7) - (self.K @ self.C)) @ self.PBar
